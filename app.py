...
from src.report_generator import FPDF_AVAILABLE, generate_report
from create_dataset import create_sample_data


DATA_DIR = Path("data")
OLD_CUSTOMER_FILE = DATA_DIR / "old_customers.csv"
...
def ensure_sample_files():
    if not OLD_CUSTOMER_FILE.exists() or not PRODUCTS_FILE.exists():
        create_sample_data()
...
# Live link rendered inside the app (detect deployment environment)
port = os.getenv("PORT", "8501")
render_url = os.getenv("RENDER_EXTERNAL_URL") or os.getenv("EXTERNAL_URL") or f"http://localhost:{port}"
st.markdown(f"**Live App Link:** [Open Dashboard]({render_url})")

old_data = load_csv(OLD_CUSTOMER_FILE)
products_data = load_csv(PRODUCTS_FILE)
...
filtered_data = apply_filters(old_data, city, gender, category)
...
if menu == "Home Dashboard":
    ...
    if filtered_data.empty:
        ...
        with col5:
            st.plotly_chart(city_bar_chart(filtered_data), width='stretch')

        with col6:
            st.plotly_chart(gender_pie_chart(filtered_data), width='stretch')

        st.subheader("Customer Data")
        st.dataframe(filtered_data, width='stretch')

    if len(low_stock_data) > 0:
        st.warning("⚠️ Low stock products found.")
    ...
    st.dataframe(old_data, width='stretch')

    uploaded_file = st.file_uploader("Upload New Customer CSV File", type=["csv"])
    ...
    if uploaded_file is not None:
        ...
        save_csv(updated_data, OUTPUTS_DIR / "updated_dataset.csv")
        ...
        st.dataframe(updated_data, width='stretch')

        st.subheader("Updated Data Visualizations")
        col1, col2 = st.columns(2)

        with col1:
            st.plotly_chart(city_bar_chart(updated_data), width='stretch')

        with col2:
            st.plotly_chart(category_pie_chart(updated_data), width='stretch')
    ...
    if filtered_data.empty:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(customer_segment_bar_chart(filtered_data), width='stretch')
        with col2:
            st.plotly_chart(customer_segment_pie_chart(filtered_data), width='stretch')

        st.subheader("K-Means 3D Visualization")
        st.plotly_chart(kmeans_3d_chart(filtered_data), width='stretch')
    ...
    if filtered_data.empty:
        ...
        st.dataframe(churn_view, width='stretch')
        ...
        st.dataframe(purchase_view, width='stretch')

        st.subheader("Prediction Visualizations")
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(churn_bar_chart(filtered_data), width='stretch')
        with col2:
            st.plotly_chart(purchase_prediction_bar_chart(filtered_data), width='stretch')
    ...
    if filtered_data.empty:
        ...
        selected_customer = filtered_data[filtered_data["customer_id"] == customer_id]
        st.subheader("Selected Customer Details")
        st.dataframe(selected_customer, width='stretch')

        recommendations = recommend_products(filtered_data, customer_id)
    ...
    st.dataframe(products_data, width='stretch')
    st.subheader("Inventory Visualizations")
    st.plotly_chart(stock_bar_chart(products_data), width='stretch')
    st.subheader("Low Stock Products")

    if len(low_stock_data) > 0:
        st.error("⚠️ Low stock products found")
        st.dataframe(low_stock_data, width='stretch')
        st.plotly_chart(low_stock_pie_chart(low_stock_data), width='stretch')
        for _, row in low_stock_data.iterrows():
            st.warning(
                f"Restock Alert: {row['product_name']} has only {row['current_stock']} items left. "
    ...
    if filtered_data.empty:
        ...
        with col3:
            st.plotly_chart(category_bar_chart(filtered_data), width='stretch')
        with col4:
            st.plotly_chart(category_pie_chart(filtered_data), width='stretch')

        col5, col6 = st.columns(2)
        with col5:
            st.plotly_chart(gender_bar_chart(filtered_data), width='stretch')
        with col6:
            st.plotly_chart(city_bar_chart(filtered_data), width='stretch')

        st.plotly_chart(age_spending_scatter(filtered_data), width='stretch')
        st.subheader("K-Means 3D Visualization")
        st.plotly_chart(kmeans_3d_chart(filtered_data), width='stretch')
    ...
    if FPDF_AVAILABLE:
        if st.button("Generate PDF Report"):
            try:
                report_path = generate_report(total_customers, total_revenue, low_stock_count)
        ...
        st.warning("PDF report generation requires the fpdf package. Install it or use the updated requirements.")
